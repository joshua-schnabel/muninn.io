...
**What survives, and it is much smaller — and unverified.** The cooldown gates
the dependency Dependabot proposes, and this repository deliberately keeps
Dependabot's scope at direct dependencies. A direct crate three days old
resolves its own requirements, so a **transitive** version published yesterday
can in principle enter `Cargo.lock` behind it, seen by neither the cooldown nor
CI, whose instruments all search for something already known.

**In principle** is the whole claim, and it is worth being exact about it after
this finding was already wrong once. Whether Dependabot's cooldown considers
transitive resolution was inferred, not read and not measured. The example this
finding first cited does not support it either: on the merged #62 the lock file
gained `clap_builder` alongside `clap`, but both were published in the *same
minute* — `clap` pins `clap_builder` exactly, so that pair shows transitives
moving, not one of them escaping the wait.

So this is a mechanism that looks possible and has not been demonstrated. The
exposure it would need is also unaimed: an attacker's freshly published crate
would have to be pulled in as a new transitive by an unrelated direct bump,
inside the three-day window, into a repository that then merges unattended.
