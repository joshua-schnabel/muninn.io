...
- **The CI/CD pipeline was audited for the first time, and four of its five findings are closed here.** `docs/security-audit.md`'s 2026-08-08 pass excluded "the CI pipeline (covered in `ci-cd.md`)" from its scope, and that exclusion had never been lifted — `ci-cd.md` describes what the pipeline does, which is not a review of what it exposes. The same pass asked to be re-run "after any change to the renderer, the Docker client, or the secret path", and since then all three moved. The document now carries both passes, newest first, with the older one unchanged.
  ...
  **M-04 is open on purpose.** Dependabot auto-merges `semver-minor` as well as `semver-patch`, unattended, into a branch whose pushes publish `:dev` images. The reasoning recorded beside it is about compatibility, and the risk that makes unattended dependency merges dangerous is a compromised release — against which `cargo deny`, Trivy and the tests are all the wrong instrument, because each looks for something already known. It is a policy decision with three options written out, not a defect, and doing nothing silently is the outcome the finding exists to prevent.
